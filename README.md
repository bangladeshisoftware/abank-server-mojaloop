...
A Bank implements the **Mojaloop FSPIOP API** to participate in an interoperable instant payment network. It exposes:
...
A Bank implements the full three-phase Mojaloop FSPIOP flow:

```
Payer DFSP (A Bank)          Mojaloop Hub              Payee DFSP
      │                           │                         │
...
When adding a merchant via `POST /api/parties/add`, A Bank:
...
When deleting a merchant, A Bank de-registers the identifier from ALS before removing local records.
...
A Bank generates ILP data dynamically per quote using standard cryptography:
...
The `fulfilment` is stored in the `transactions` table against the `quote_id`. When the transfer arrives (`POST /transfers`), A Bank retrieves the stored fulfilment and includes it in the `PUT /transfers/:id` COMMITTED response, ensuring cryptographic proof of payment.

---

## Email Notifications

On merchant creation, A Bank sends a styled HTML welcome email containing:
...
A Bank follows defensive patterns throughout:
